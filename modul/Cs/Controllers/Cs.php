...
class Cs extends BaseController
{
    ...
    public function datatable()
    {
        ...
        if($id_mitra != 0){
            $builder = $this->db->table('callsenter c')
            ->select('u.id as iduser,u.nama_user as nama, u.no_hp as hp,u.email as email,c.status as stt')
            ->join('user u','u.id = c.id_user','LEFT')
            ->where("c.id_mitra = $id_mitra")
